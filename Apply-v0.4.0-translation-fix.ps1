...
Get-ChildItem -LiteralPath $PackageRoot -Force |
    Where-Object {
        $_.Name -ne "Apply-v0.4.0-translation-fix.ps1"
    } |
...
if (-not [string]::IsNullOrWhiteSpace($changes)) {
    Invoke-Checked {
        git commit -m "Fix silent translation startup failure"
    } "git commit"
...
Write-Host ""
Write-Host "Inicio real de traducción corregido y PR #4 actualizado." -ForegroundColor Green
