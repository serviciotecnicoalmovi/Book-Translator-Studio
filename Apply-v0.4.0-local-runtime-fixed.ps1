...
Get-ChildItem -LiteralPath $PackageRoot -Force |
    Where-Object {
        $_.Name -ne "Apply-v0.4.0-local-runtime-fixed.ps1"
    } |
...
if (-not [string]::IsNullOrWhiteSpace($changes)) {
    Invoke-Checked {
        git commit -m "Fix local model download startup and progress"
    } "git commit"
...
Write-Host ""
Write-Host "Descarga local corregida y PR #4 actualizado." -ForegroundColor Green
