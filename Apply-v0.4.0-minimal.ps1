...
Get-ChildItem -LiteralPath $PackageRoot -Force |
    Where-Object { $_.Name -ne "Apply-v0.4.0-minimal.ps1" } |
    Copy-Item -Destination $RepositoryPath -Recurse -Force

Invoke-Checked { dotnet restore BookTranslatorStudio.sln } "dotnet restore"
Invoke-Checked {
...
if (-not [string]::IsNullOrWhiteSpace($changes)) {
    Invoke-Checked {
        git commit -m "Fix minimal translation workspace compilation"
    } "git commit"
...
Write-Host ""
Write-Host "Interfaz esencial compilada y PR #4 actualizado correctamente." -ForegroundColor Green
