...
Get-ChildItem -LiteralPath $PackageRoot -Force |
    Where-Object { $_.Name -ne "Apply-v0.4.0-open-book.ps1" } |
...
if (-not [string]::IsNullOrWhiteSpace($changes)) {
    Invoke-Checked {
        git commit -m "Restore unified PDF and project opening"
    } "git commit"
...
Write-Host ""
Write-Host "Botón Abrir libro compilado y PR #4 actualizado correctamente." -ForegroundColor Green
