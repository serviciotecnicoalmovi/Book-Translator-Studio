...
Write-Host "Restaurando la rama de trabajo..." -ForegroundColor Cyan
...
Write-Host "Aplicando motor local corregido..." -ForegroundColor Cyan
Get-ChildItem -LiteralPath $PackageRoot -Force |
    Where-Object {
        $_.Name -ne "Apply-v0.4.0-local-fixed.ps1"
    } |
...
if (-not [string]::IsNullOrWhiteSpace($changes)) {
    Invoke-Checked {
        git commit -m "Fix local translation runtime compilation"
    } "git commit"
...
Write-Host ""
Write-Host "Motor local corregido, compilado y PR #4 actualizado." -ForegroundColor Green
