...
Get-ChildItem -LiteralPath $PackageRoot -Force |
    Where-Object {
        $_.Name -ne "Apply-v0.4.0-start-translation.ps1"
    } |
...
if (-not [string]::IsNullOrWhiteSpace($changes)) {
    Invoke-Checked {
        git commit -m "Fix first translation startup configuration"
    } "git commit"
...
Write-Host ""
Write-Host "Inicio de traducción corregido y PR #4 actualizado." -ForegroundColor Green
