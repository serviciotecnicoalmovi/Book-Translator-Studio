...
Get-ChildItem -LiteralPath $PackageRoot -Force |
    Where-Object {
        $_.Name -ne "Apply-v0.4.0-local-cancel.ps1"
    } |
...
if (-not [string]::IsNullOrWhiteSpace($changes)) {
    Invoke-Checked {
        git commit -m "Add cancellation to local translation workflow"
    } "git commit"
...
Write-Host ""
Write-Host "Botón Cancelar compilado y PR #4 actualizado." -ForegroundColor Green
