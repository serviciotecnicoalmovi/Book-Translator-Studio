...
Get-ChildItem -LiteralPath $PackageRoot -Force |
    Where-Object {
        $_.Name -ne "Apply-v0.4.0-local-speed.ps1"
    } |
...
if (-not [string]::IsNullOrWhiteSpace($changes)) {
    Invoke-Checked {
        git commit -m "Optimize local translation throughput"
    } "git commit"
...
Write-Host ""
Write-Host "Traducción local optimizada y PR #4 actualizado." -ForegroundColor Green
